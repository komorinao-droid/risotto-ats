...
- Slot (Phase K) / Base (Phase L) / Client (Phase M) / 設定系 10 画面 (Phase N) は完了済。さらに整理タスク O-1 (baseScope.ts dead helper 削除) / O-3 (AddApplicantModal の `updateClientData` 撤去) / O-4 (AuthContext `updateClientData` shim 本体削除) / O-5 (baseScope.ts ファイル全体削除) / O-6a (AdminApp read-only 5 callsite を `clientDataRepository.get` 化) / O-6d (AdminApp delete cascade を `clientDataRepository.delete` + `clearClientLogs` 化) / O-6b (invoiceRepository 新規追加 + AdminApp invoice CRUD 6 callsite を `invoiceRepository.upsert/bulkUpsert/findByYearMonth` 化、AdminApp から `storage.*` 直叩き完全消滅) も完了。**残る直書き経路**は **AdminApp の tenant copy (O-6c: raw `localStorage.getItem/setItem('hireflow:client:...:data')` 5 callsite)** および **AdminAccount / AdminLog / MediaIntegration、媒体連携、契約・請求等** のみ。Firestore 化と同時に立ち上げると PR が肥大化するので、**段階順序を守ること**。
...
| **O-6c** | tenant copy 系の raw `localStorage` 直叩き撤去 | 残: BulkApplicantCopy / BulkSettingsCopy の `localStorage.getItem/setItem('hireflow:client:${id}:data', ...)` 5 callsite を `clientDataRepository.get/save` 経由に | ★ |
