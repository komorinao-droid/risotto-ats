...
- Slot (Phase K) / Base (Phase L) / Client (Phase M) / 設定系 10 画面 (Phase N) は完了済。さらに整理タスク O-1 (baseScope.ts dead helper 削除) / O-3 (AddApplicantModal の `updateClientData` 撤去) / O-4 (AuthContext `updateClientData` shim 本体削除) / O-5 (baseScope.ts ファイル全体削除) / O-6a (AdminApp read-only 5 callsite を `clientDataRepository.get` 化) も完了。**残る直書き経路**は **AdminApp の write path (O-6b〜d: invoice / tenant copy / delete cascade)** および **AdminAccount / AdminLog / MediaIntegration、媒体連携、契約・請求等** のみ。Firestore 化と同時に立ち上げると PR が肥大化するので、**段階順序を守ること**。
...
| **O-6d** | delete cascade を `clientDataRepository.delete` + `clientLog.clearClientLogs` に整理 | 残: `handleDelete` の `storage.deleteClientData` 1 callsite + `localStorage.removeItem('hireflow:client:${id}:logs')` 2 callsite | ★ |
