...
- Slot (Phase K) / Base (Phase L) / Client (Phase M) / 設定系 10 画面 (Phase N) は完了済。さらに整理タスク O-1 (baseScope.ts dead helper 削除) / O-3 (AddApplicantModal の `updateClientData` 撤去) / O-4 (AuthContext `updateClientData` shim 本体削除) / O-5 (baseScope.ts ファイル全体削除) / O-6 (a: read-only / b: invoice CRUD / c: tenant copy / d: delete cascade) **全 4 サブフェーズ完了**。**`src/admin` および `src/client` 配下から `storage.{getClientData,saveClientData,deleteClientData}` 直叩きおよび raw `localStorage.{get,set,remove}Item('hireflow:client:...')` 直叩きが完全消滅し、すべて Repository / `clientLog.ts` 経由になった。** 残る直書き経路は **AdminAccount / AdminLog / MediaIntegration、媒体連携、契約・請求等** のみ。Firestore 化と同時に立ち上げると PR が肥大化するので、**段階順序を守ること**。
...
| **O-6a** | AdminApp read-only path の `clientDataRepository` 化 | ✅ 完了（read-only 5 callsite を `storage.getClientData` → `clientDataRepository.get` に置換: `clientStats.ts:59` / `AdminApp.tsx:2318` (ClientDetail useMemo) / `AdminApp.tsx:3550` (BulkInvoiceModal preview) / `AdminApp.tsx:3845` (invoiceByClient useMemo) / `RecruitmentReportPrint.tsx:39`。try/catch / useMemo dependency / fallback 挙動は維持。`clientStats.ts` と `RecruitmentReportPrint.tsx` からは `storage` import を撤去）| — |
| **O-6b** | invoice 系 read/write path を新規 `invoiceRepository` 経由化 | ✅ 完了（`InvoiceRepository` interface に `list` / `findByYearMonth` / `upsert` / `bulkUpsert` の 4 API を追加、`LocalStorageInvoiceRepository` で実装（`InvoiceLine[]` まで 2 階層 deep copy / id 採番は呼出側責務）。AdminApp の 6 callsite を置換: InvoiceSection.onUpdateInvoices (`bulkUpsert`) / BulkInvoiceModal preview (`findByYearMonth`) / BulkInvoiceModal.handleRun (`upsert`) / openInvoiceEditor (`findByYearMonth`) / handleSingleSave (`upsert`) / invoiceByClient useMemo (`findByYearMonth`)。AdminApp.tsx から `storage` import 撤去、`storage.{getClientData,saveClientData,deleteClientData}` 直叩きは AdminApp 全体で 0 件）| — |
| **O-6c** | tenant copy 系の raw `localStorage` 直叩き撤去 | ✅ 完了（`BulkApplicantCopy` の `loadAppData` / `handleAppExecute` save、`BulkSettingsCopy.handleExecute` の src read / dst read / dst save の 5 callsite を `clientDataRepository.get/save` 経由に置換。cross-tenant でも 1 client ずつ `get/save` するだけなのでインターフェース変更なし。masking / copyItems / id 採番 / alert / onLog / state 更新は不変。`tenantCopyService` 等の新規抽象化は見送り — Firestore 化フェーズで WriteBatch + Cloud Functions と一緒に検討。**これで Phase O-6 (a/b/c/d) 全完了 / AdminApp / client から `storage.*ClientData` および raw `localStorage.*hireflow:client` 直叩きが完全消滅**）| — |
| **O-6d** | delete cascade を `clientDataRepository.delete` + `clientLog.clearClientLogs` に整理 | ✅ 完了（`AdminApp.handleDelete` で `storage.deleteClientData(c.id)` → `clientDataRepository.delete(c.id)`、`localStorage.removeItem('hireflow:client:${c.id}:logs')` および子アカ forEach 経路 → `clearClientLogs(c.id)` / `clearClientLogs(ch.id)`。`clearClientLogs` は `src/utils/clientLog.ts` に既存。parent 限定 / try-catch / confirm dialog / selectedClientId 判定 / logAdminAction は維持）| — |
