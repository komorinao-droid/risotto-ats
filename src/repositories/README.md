...
- **AdminApp read-only path の `clientDataRepository` 化 (O-6a)**: ✅ 完了。`storage.getClientData` 直叩きのうち read-only 5 callsite (`clientStats.ts:59` / `AdminApp.tsx:2318` (ClientDetail useMemo) / `AdminApp.tsx:3550` (BulkInvoiceModal preview) / `AdminApp.tsx:3845` (invoiceByClient useMemo) / `RecruitmentReportPrint.tsx:39`) を `clientDataRepository.get` に置換。try/catch / useMemo dependency / fallback 挙動は維持。`clientStats.ts` と `RecruitmentReportPrint.tsx` からは `storage` import を撤去（read-only 単独利用だったため）。残る write path / delete cascade callsite は O-6b〜d で対応
- **AdminApp delete cascade の `clientDataRepository` + `clientLog.clearClientLogs` 化 (O-6d)**: ✅ 完了。`handleDelete` の 3 callsite を置換: `storage.deleteClientData(c.id)` → `clientDataRepository.delete(c.id)`、`localStorage.removeItem('hireflow:client:${c.id}:logs')` → `clearClientLogs(c.id)`、子アカ forEach も `clearClientLogs(ch.id)` に変更。`clearClientLogs` は `src/utils/clientLog.ts` に既存（追加不要）。`parent` 限定の cascade / try-catch / confirm dialog / selectedClientId 判定 / logAdminAction は維持。これで AdminApp.tsx から `:logs` キーの raw `localStorage.removeItem` は完全消滅。残るのは O-6b (invoice write) と O-6c (tenant copy `:data`)
- **`invoiceRepository` 新規追加 + AdminApp invoice CRUD 移行 (O-6b)**: ✅ 完了。新規 `InvoiceRepository` interface (`list` / `findByYearMonth` / `upsert` / `bulkUpsert`) を `src/repositories/types.ts` に追加し、`src/repositories/localStorage/invoiceRepository.ts` で `LocalStorageInvoiceRepository` 実装（`InvoiceLine[]` まで 2 階層 deep copy、id 採番は呼出側責務）。AdminApp.tsx の 6 callsite を置換: `InvoiceSection.onUpdateInvoices` (mutator → `bulkUpsert`) / `BulkInvoiceModal` preview existing 取得 (`findByYearMonth`) / `BulkInvoiceModal.handleRun` (per-row write → `upsert`) / `openInvoiceEditor` (`findByYearMonth`) / `handleSingleSave` (write → `upsert`) / `invoiceByClient` useMemo (`findByYearMonth`)。これにより AdminApp.tsx から `storage.{getClientData,saveClientData,deleteClientData}` 直叩きが完全消滅し、`storage` import も撤去。id 採番ロジック / try-catch / logAdminAction / confirm 挙動 / UI 文言は変更なし。残る AdminApp の raw storage 直叩きは O-6c の tenant copy `:data` 5 callsite のみ
- **AdminApp tenant copy paths の `clientDataRepository` 化 (O-6c)**: ✅ 完了。`BulkApplicantCopy` / `BulkSettingsCopy` の raw localStorage 直叩き 5 callsite (`loadAppData` / `handleAppExecute` save / `handleExecute` src read / dst read / dst save) を `clientDataRepository.get/save` 経由に置換。cross-tenant (src + dst の 2 client を同時に触る) でも 1 client ずつ `get/save` を呼ぶだけなのでインターフェース変更なし。masking (`maskApplicant`) / `copyItems` checkbox / id 採番 (`baseId = max(existingIds) + 1`) / alert メッセージ / `onLog` 呼出 / 容量上限エラー文言は変更なし。`tenantCopyService` 等の新規 service 抽象化は見送り（Firestore 化フェーズで WriteBatch + Cloud Functions 設計と一緒に再検討）。**これで Phase O-6 (a/b/c/d) の 4 サブフェーズが全て完了し、AdminApp.tsx および `src/admin` / `src/client` 配下から `storage.{getClientData,saveClientData,deleteClientData}` 直叩きおよび raw `localStorage.{get,set,remove}Item('hireflow:client:...')` 直叩きが完全消滅した。**残るのは Repository 内部実装 / `storage.ts` / `clientLog.ts` の 3 ファイルのみ（境界として正しい配置）
