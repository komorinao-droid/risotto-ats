            ...
            <div style={{ display: 'flex', gap: '1rem', flexWrap: 'wrap', alignItems: 'flex-start' }}>
              ...
              <div style={{ flex: 1, minWidth: '300px' }}>
                <table style={tableStyle}>
                  <thead>
                    <tr>
                      <th style={thStyle}>NG理由 / サブステータス</th>
                  ...
                  <tbody>
                    {ngBreakdown.reasons.map((row) => {
                      const reasonColor: Record<string, string> = {
                        age: '#F59E0B', condition: '#3B82F6', duplicate: '#9CA3AF',
                        personality: '#DC2626', other: '#6B7280',
                      };
                      const color = reasonColor[row.key];
                      return (
                        <React.Fragment key={row.key}>
                          ...
                          <tr>
                            ...
                            <td style={{ ...tdStyle, textAlign: 'right', color: '#6B7280' }}>{ngBreakdown.total > 0 ? `${((row.count / ngBreakdown.total) * 100).toFixed(1)}%` : '-'}</td>
                          </tr>
                          {/* サブステータス内訳行（サブが2つ以上ある場合のみインデント表示） */}
                          {row.subRows.map((sub) => (
                            <tr key={`${row.key}-${sub.subStatus}`}>
                              <td style={{ ...tdStyle, paddingLeft: '2rem', color: '#6B7280' }}>
                                <span style={{ color: '#D1D5DB', marginRight: '0.5rem' }}>└</span>
                                <span style={{ display: 'inline-block', padding: '0.0625rem 0.5rem', borderRadius: '999px', fontSize: '0.6875rem', backgroundColor: '#FEF3C7', color: '#92400E' }}>
                                  {sub.subStatus}
                                </span>
                              </td>
                              <td style={{ ...tdStyle, textAlign: 'right' }}>{fmt(sub.count)}名</td>
                              <td style={{ ...tdStyle, textAlign: 'right', color: '#9CA3AF' }}>{sub.rate.toFixed(1)}%</td>
                            </tr>
                          ))}
                        </React.Fragment>
                      );
