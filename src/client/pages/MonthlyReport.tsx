...
function getWeeksOfMonth(year: number, month: number): { start: Date; end: Date; label: string }[] {
  ...
  while (cur <= last || weeks.length === 0) {
    ...
    weeks.push({ start: weekStart, end: weekEnd, label: `W${weekNum} (${s}-${e})` });
...
const BarChart: React.FC<BarChartProps> = ({ data, title, barColor = '#3B82F6', height = 260, svgRef }) => {
  ...
  const chartW = Math.max(data.length * 64 + marginLeft + marginRight, 300);
  const chartH = height;
  const plotH = chartH - marginTop - marginBottom;
  const plotW = chartW - marginLeft - marginRight;
  const barWidth = Math.min(40, plotW / data.length - 8);
...
const MonthlyReport: React.FC = () => {
  ...
  // Filtered applicants
  const monthApplicants = useMemo(() => {
    if (!clientData) return [];
    return clientData.applicants.filter((a) => isInMonth(a.date, year, month));
  ...
  const weeklyData = useMemo(() => {
    return weeks.map((w) => ({
      label: w.label,
      value: monthApplicants.filter((a) => isInDateRange(a.date, w.start, w.end)).length,
    }));
  }, [weeks, monthApplicants]);

  // Sources
  const sources = useMemo(() => (clientData ? clientData.sources : []), [clientData]);

  // Source x week table data
  const sourceWeekData = useMemo(() => {
    return sources.map((src) => {
      const weekCounts = weeks.map(
        (w) =>
          monthApplicants.filter(
            (a) => a.src === src.name && isInDateRange(a.date, w.start, w.end)
          ).length
      );
      const total = weekCounts.reduce((s, v) => s + v, 0);
      return { name: src.name, weekCounts, total };
    });
  }, [sources, weeks, monthApplicants]);

  // CPA data
  const cpaData = useMemo(() => {
    return sources
      .map((src) => {
        const count = monthApplicants.filter((a) => a.src === src.name).length;
        const cpa = count > 0 ? src.monthlyCost / count : 0;
        return { name: src.name, count, cost: src.monthlyCost, cpa, color: src.color };
      })
      .filter((d) => d.cost > 0 || d.count > 0);
  }, [sources, monthApplicants]);

  // CSV export - overview
  const exportOverviewCsv = useCallback(() => {
    const header = ['媒体名', ...weeks.map((w) => w.label), '合計'].join(',');
    const rows = sourceWeekData.map(
      (d) => [d.name, ...d.weekCounts, d.total].join(',')
    );
    const totalRow = [
      '合計',
      ...weeks.map((_, i) => sourceWeekData.reduce((s, d) => s + d.weekCounts[i], 0)),
      sourceWeekData.reduce((s, d) => s + d.total, 0),
    ].join(',');
    const csv = [header, ...rows, totalRow].join('\n');
    downloadCsv(`月次レポート_${year}年${month}月.csv`, csv);
  }, [weeks, sourceWeekData, year, month]);

  // CSV export - CPA
  const exportCpaCsv = useCallback(() => {
    const header = '媒体名,応募数,月額費用,CPA,評価';
    const rows = cpaData.map((d) => {
      const rating = d.cpa > 0 ? getCpaRating(d.cpa).label : '-';
      return [d.name, d.count, d.cost, Math.round(d.cpa), rating].join(',');
    });
    const csv = [header, ...rows].join('\n');
    downloadCsv(`CPA分析_${year}年${month}月.csv`, csv);
  }, [cpaData, year, month]);

  // PNG export
  const exportCpaPng = useCallback(() => {
    if (cpaChartRef.current) {
      svgToPng(cpaChartRef.current, `CPA分析_${year}年${month}月.png`);
    }
  }, [year, month]);
  ...
  const exportMainChartPng = useCallback(() => {
    if (mainChartRef.current) {
      svgToPng(mainChartRef.current, `週別応募数_${year}年${month}月.png`);
    }
  }, [year, month]);
  ...
  return (
    <div style={pageStyles.container}>
      ...
      <div style={pageStyles.header}>
        <h2 style={pageStyles.title}>月次レポート</h2>
        <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', flexWrap: 'wrap' }}>
          ...
          <div style={pageStyles.nav}>
            ...
            <span style={pageStyles.navLabel}>{year}年{month}月</span>
      ...
      {activeTab === 'overview' && (
        <>
          ...
          <div style={pageStyles.section}>
            <BarChart svgRef={mainChartRef} data={weeklyData} title="週別応募数" />
          </div>
              ...
              <table>
                <thead>
                  <tr>
                    <th>媒体名</th>
                    {weeks.map((w, i) => (
                      <th key={i} style={{ textAlign: 'center' }}>{w.label}</th>
                    ))}
                    <th style={{ textAlign: 'center' }}>合計</th>
                  </tr>
                </thead>
                <tbody>
                  {sourceWeekData.map((d) => (
                    <tr key={d.name}>
                      <td>{d.name}</td>
                      {d.weekCounts.map((c, i) => (
                        <td key={i} style={{ textAlign: 'center' }}>{c}</td>
                      ))}
                      <td style={{ textAlign: 'center', fontWeight: 600 }}>{d.total}</td>
                    </tr>
                  ))}
                  {sourceWeekData.length === 0 && (
                    <tr>
                      <td colSpan={weeks.length + 2} style={{ textAlign: 'center', color: '#9ca3af' }}>
                        データがありません
                      </td>
                    </tr>
                  )}
                    ...
                    <tr style={{ fontWeight: 600, backgroundColor: '#F9FAFB' }}>
                      ...
                      {weeks.map((_, i) => (
                        <td key={i} style={{ textAlign: 'center' }}>
                          {sourceWeekData.reduce((s, d) => s + d.weekCounts[i], 0)}
                        </td>
                      ))}
      ...
      {activeTab === 'cpa' && (
        <>
          ...
          <div style={pageStyles.section}>
            <div style={pageStyles.sectionHeader}>
              <span style={pageStyles.sectionTitle}>媒体別CPA</span>
              ...
              <table>
                <thead>
                  <tr>
                    ...
                    <th style={{ textAlign: 'right' }}>月額費用</th>
